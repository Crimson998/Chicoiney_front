...
const Crash = () => {
  ...
  const handleStart = async () => {
    ...
    try {
      ...
      if (DEBUG) {
        console.log('[Crash] Backend created_at:', res.data.created_at);
        console.log('[Crash] Client now:', new Date().toISOString());
        console.log('[Crash] Crash multiplier:', res.data.crash_multiplier);
      }
      setRoundId(res.data.id);
      setCrashMultiplier(res.data.crash_multiplier);
      setCreatedAt(res.data.created_at);
      setServerSeedHash(res.data.server_seed_hash || null);
      setServerSeed(null);
      setGameActive(true);
      setMultiplier(1.0);
  ...
  const animateMultiplier = (crashAt, createdAtStr) => {
    ...
    if (DEBUG) {
      console.log('[Crash] animateMultiplier: createdAtStr:', createdAtStr, 'createdAtTime:', createdAtTime, 'now:', Date.now(), 'diff(ms):', Date.now() - createdAtTime);
    }
    ...
    intervalRef.current = setInterval(() => {
      ...
      if (DEBUG) {
        console.log('[Crash] interval: now:', now, 'elapsed:', elapsed, 'current:', current, 'crashAt:', crashAt, 'crashTime:', crashTime);
      }
  ...
  const handleCashout = async () => {
    ...
    try {
      ...
      if (createdAt) {
        ...
        if (DEBUG) {
          console.log('[Crash] Cashout: createdAt:', createdAt, 'createdAtTime:', createdAtTime, 'now:', now, 'elapsed:', elapsed, 'cashoutMultiplier:', cashoutMultiplier);
        }
      }
  ...
  const handleCrash = async () => {
    ...
    try {
      ...
      setCredits(null);
      setServerSeed(res.data.server_seed || null);
      setServerSeedHash(res.data.server_seed_hash || serverSeedHash);
    } catch (err) {
      // ignore
    }
  };
      ...
      <div style={{ margin: '2rem auto', maxWidth: 500, background: 'rgba(255,255,255,0.05)', borderRadius: 16, padding: 32 }}>
        ...
        {serverSeedHash && (
          <div style={{ marginBottom: 8, fontSize: 12, color: '#aaa' }}>
            <b>Provably Fair Hash:</b> {serverSeedHash}
          </div>
        )}
        {/* Server seed reveal after round */}
        {serverSeed && (
          <div style={{ marginBottom: 8, fontSize: 12, color: '#aaa' }}>
            <b>Server Seed:</b> {serverSeed}
          </div>
        )}
